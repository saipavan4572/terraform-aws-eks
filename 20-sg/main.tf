module "db" {
  source = "git::https://github.com/saipavan4572/terraform-aws-securitygroup?ref=main"
...
## node_cluster and cluster_node are in private subnet and both are communicating with each other continuously.


# Node accepting traffic from Ingress
resource "aws_security_group_rule" "node_ingress" {
  type              = "ingress"
  from_port         = 0
  to_port           = 65535
  #from_port         = 30000                     # production we can mention a specific port to enable
  #to_port           = 32768                     # ephemeral range in eks
...
# Ingress ALB accepting traffic on 443 - https
...
# Ingress ALB accepting traffic on 80 - http
